...
async function initApp() {
  try {
    ...
    // Initial Renders
    const isVolunteers = window.location.pathname.toLowerCase().includes('volunteers.html');
    showPage(isVolunteers ? "volunteers_home" : "home");
...
async function showPage(pageId) {
  try {
    // Hide all currently active search states if any (optional)
    ...
    if (res.ok) {
      ...
      if (container) {
        ...
        // Re-initialize specific components based on page loaded
        if (pageId === "home") {
          renderUpcomingList("upcomingList");
        } else if (pageId === "Course_Catalogue") {
          initCalendar();
        } else if (pageId === "Course_Catalogue1") {
          renderCatalogue(globalRawClasses, globalRawDescs);
          setupAlphabetNav();
        } else if (pageId === "Learner_VV") {
          renderVideoVault(globalVideoVault);
        } else if (pageId === "QI_Page") {
          renderQI(globalQualityImprovement);
        } else if (pageId === "Training_Rooms") {
          setupRoomSearch();
          filterRooms();
        }
      }
...
function searchPages() {
  // Since pages are now fetched dynamically, searching across all pages requires a different strategy.
  // We can either fetch and search, or limit search to the course catalogue and current page.
  // For now, we will redirect to course catalogue search.
  const query = document.getElementById("searchInput").value.toLowerCase();
  if (query !== "") {
    showPage("Course_Catalogue1").then(() => {
       const pSearch = document.getElementById("prospectusSearchInput");
       if (pSearch) {
           pSearch.value = query;
           filterProspectus(query);
       }
    });
  }
}
